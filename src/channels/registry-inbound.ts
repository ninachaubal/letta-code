  ...
  async function handleInboundMessage(
    ...
    if (senderAccess === "deny") {
      ...
      if (resolveChannelAccessScope(msg.chatType) === "dm") {
        await adapter.sendDirectReply(
          ...
          // Direct-reply options keep the denial inside the private bot
          // topic the user messaged from.
    ...
    if (senderAccess === "pair") {
      ...
      await adapter.sendDirectReply(
        ...
        // Direct-reply options keep the pairing instructions inside the
        // private bot topic the user messaged from.
        buildDirectReplyOptions(msg),
      );
      return;
    }
